...
認證方式與通用模式請參考 `api-common.md`。

---

## 1. POST /issues.json — 建立議題
...
## 2. GET /issues.json — 查詢議題列表

```bash
curl -s -H "X-Redmine-API-Key: ${API_KEY}" \
  "${URL}/issues.json?project_id=project-x&status_id=open&assigned_to_id=me&sort=updated_on:desc&limit=25"
```
...
```bash
curl -s -H "X-Redmine-API-Key: ${API_KEY}" \
  "${URL}/issues.json?project_id=project-x&include=journals,relations,attachments"
```
...
## 3. GET /issues/{id}.json — 查詢單一議題

```bash
curl -s -H "X-Redmine-API-Key: ${API_KEY}" \
  "${URL}/issues/123.json?include=children,attachments,relations,changesets,journals,watchers"
```
...
- 最常見用途：更新狀態 + 加入備註
- 成功回應 HTTP 200，body 為空

---

## 5. DELETE /issues/{id}.json — 刪除議題
...
- `Content-Type` 在 Step 1 必須設為 `application/octet-stream`
...
| 「issue #200 加上附件 report.pdf」 | 附件兩步驟流程（uploads → PUT） |
